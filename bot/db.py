...
sticky_messages one persistent sticky configuration per Discord channel.
...
CREATE INDEX IF NOT EXISTS idx_giveaways_guild_open
...
class Database:
    ...
    async def all_sticky_channel_ids(self) -> set[int]:
        cur = await self.conn.execute("SELECT channel_id FROM sticky_messages")
    ...
    async def giveaway_by_message(self, message_id: int) -> aiosqlite.Row | None:
    ...
    async def giveaway_entries(self, gid: int) -> list[aiosqlite.Row]:
        async with self.conn.execute(
            "SELECT * FROM giveaway_entries WHERE giveaway_id = ? ORDER BY joined_at", (gid,)
        ) as cur:
            return list(await cur.fetchall())

    async def count_giveaway_entrants(self, gid: int) -> int:
        async with self.conn.execute(
